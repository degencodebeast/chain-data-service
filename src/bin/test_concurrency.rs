    ...
    async fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let state_test = tokio::spawn(async move {
            ...
            let cache_key = chain_data_service::cache::CacheKey::transaction_query(&key, 0, 0, 0, 0);
            cache_guard.transaction_cache.insert(cache_key.clone(), (value.clone(), 1), None).await;
            let retrieved = cache_guard.transaction_cache.get(&cache_key).await;
            assert!(retrieved.is_some(), "Cache entry should exist");
            assert_eq!(retrieved.unwrap().0[0].signature, "test_concurrent_1");
